...
var Park = function (_Element) {
	...
	function Park(name, buildYear, area, numberOfTrees) {
		...
		_this.area = area;
		_this.numbersOfTrees = numberOfTrees;
		return _this;
	}
	...
	_createClass(Park, [{
		...
		value: function treeDensity() {
			var density = this.numbersOfTrees / this.area;
			console.log(this.name + ' has a tree density of ' + density + ' trees per sq mile.');
		}
	}]);
...
function calc(arr) {
	//reduce the array to a single value, add prev val to cur value, starting at elem 0
	var sum = arr.reduce(function (prev, curr, indx) {
		return prev + curr;
	}, 0);
...
var allParks = [
	new Park('Green Park', 1987, .02, 215), 
	new Park('National Park', 1894, 2.9, 3549), 
	new Park('Oak Park', 1953, 0.4, 949)];

var allStreets = [
	new Street('Ocean Ave', 1991, 1.1, 4),
	new Street('Everygreen St', 2008, 2.7, 2), 
	new Street('4th Street', 2015, 0.8), 
	new Street('Sunset Blvd.', 1982, 6, 5)];

function reportParks(p) {
	console.log('-------- Parks Report');
	//Density
	p.forEach(function (curr) {
		return curr.treeDensity();
	});
	//Average Age
	var ages = p.map(function (curr) {
		return new Date().getFullYear() - curr.buildYear;
	});
	...
	console.log('Our ' + p.length + ' parks have an average of ' + averageAge);
	var i = p.map(function (elem) {
		return elem.numbersOfTrees;
...
function reportStreets(s) {
	...
	var _calc3 = calc(s.map(function (el) {
	    ...
	    averageLength = _calc4[1];

	console.log('Our ' + s.length + ' streets have a total length of ' + totalLength + ' and an average length of ' + averageLength + ' miles');
	//5. Size classification of all streets: tiny/small/normal/big/huge. If the size is unknown, the default is normal
